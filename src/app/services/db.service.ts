...
export class DbService implements OnInit, OnChanges {
  ...
  rooturi = 'http://192.168.1.109:8080/';
  ...
  public showMessage(
    ...
    if (this.isObject(message) && message.status === 0) {
      ...
      for (const key in message) {
        if (message[key]) {
          formattedMessage += `${key}: ${message[key].join(', ')}<br>`;
        }
        if (formattedMessage) {
  ...
  downloadFile(
    ...
    this.showloaderfunction(loader);
    let fullUrl = url.includes('http') ? url : `${this.ServiceURL}${url}`;

    const token = this.getToken();

    const headers = new HttpHeaders({
      Authorization: `Bearer ${token}`,
      // 'Content-Type': 'application/json', // Not needed for Blob type
    });

    const requestOptions = {
      headers: headers,
      params: data || {}, // Ensure params is always an object
      responseType: 'blob' as 'json', // Forcing TypeScript to accept 'blob' as a valid JSON responseType
    };

    this.http.get<Blob>(fullUrl, requestOptions).subscribe(
      (blob) => {
        this.hideLoaderfunction(loader);
        success?.(blob); // Pass the blob directly to success callback
      },
      (error) => {
        this.hideLoaderfunction(loader);
        this.showMessage(error);
        if (
          ...
          fail?.(error); // Invoke the fail callback if it's defined
        }
      }
    );
  }
