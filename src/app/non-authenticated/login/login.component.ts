...
export class LoginComponent implements OnInit, OnDestroy {
  ...
  loginData: any = { username: '', password: '', remember: false, cpatchaTextBox: '' };
  fp: any = { emailormobile: '', isemail: '', cpatchaTextBox: '' };
  resetpassword: any = { otp: '', newpassword: '', confirmpassword: '', emailormobile: '' };
  otp = { otp: '' };
  currentUrl:string;
  ...
  Captchacode: string;
  salt: string;
  constructor(private router: Router, public db: DbService, public formBuilder: UntypedFormBuilder, private route: ActivatedRoute, private saltService: SaltService) {
  ...
  async login() {
    ...
    if (this.loginData.cpatchaTextBox == this.Captchacode) {
      const key = await this.saltService.getSalt().then(salt => this.salt = salt);
      const encryptedPassword = await this.saltService.encryptPassword(key, this.loginData.password); //this.loginData.password;//
      // this.loginData.password = btoa(this.loginData.password);
      debugger;
      const data = { email: this.loginData.username, password: encryptedPassword, remember: this.loginData.remember };

      this.db.post('api/authenticate/', data, ((response): void => {
        debugger;
  ...
  ForgetPassword(): void {
    ...
    if (this.fp.cpatchaTextBox == this.Captchacode) {
      const data = {
        ...
        isemail: 1, email: this.fp.emailormobile
      };
  ...
  createCaptcha(): void {
    ...
    var charsArray =
      "0123456789abdefghijklmnqrtuABCDEFGHIJKLMNOPQRSTUVWXYZ";
    var lengthOtp = 6;
    var captcha = [];
  ...
  async checkotp() {
    const key = await this.saltService.getSalt().then(salt => this.salt = salt);
    const encryptedOTP = await this.saltService.encryptPassword(key, this.otp.otp);
  ...
  async changepasswordFN() {
    ...
    if (this.resetpassword.newpassword.length >= 6 && this.resetpassword.confirmpassword == this.resetpassword.newpassword) {
      ...
      const key = await this.saltService.getSalt().then(salt => this.salt = salt);
      const newencryptedPassword = await this.saltService.encryptPassword(key, this.resetpassword.newpassword);
      const confirmencryptedPassword = await this.saltService.encryptPassword(key, this.resetpassword.confirmpassword);
      const encryptedOTP = await this.saltService.encryptPassword(key, this.resetpassword.otp);
      ...
      this.db.post('api/reset-password/'+this.uid+'/'+this.token+'/' , data, (response): void => {
  ...
  BackToLogin() {
    ...
    this.showChangepassword = false;
    this.showOtp = false;
    this.loginData = { username: '', password: '', remember: false, cpatchaTextBox: '' };
    this.fp = { emailormobile: '', isemail: '', cpatchaTextBox: '' };
    this.resetpassword = { otp: '', newpassword: '', confirmpassword: '', emailormobile: '' };
    this.otp = { otp: '' };
    setTimeout(() => {
      this.createCaptcha();
    }, 100);
