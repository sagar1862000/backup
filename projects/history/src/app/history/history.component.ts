...
export class HistoryComponent implements OnInit {
  ...
  DataDefinitions = [
    ...
    {
      headerName: 'currentOrganization',
      field: 'currentOrganization',
      type: 'texticon',
      iconname: 'organization.svg',
      size: 'col-md-12',
      show: 'yes',
      print: 'bodyright',
      showxl: 'yes',
  ...
  columnDefsexcel = [
    ...
    },
    {
      ...
      cellRenderer(param) {
        return (
          `<button type='button' data-action-type='candidateshow' data-toggle="modal" class='btn  btn-sm'>
          ` +
          param.value +
          `
    </button>
...
`
    ...
    },
    {
      ...
      cellRenderer(param) {
        return (
          `<button type='button' help='` +
          param.value +
          `'  data-action-type='status' data-toggle="modal" class='btn  btn-sm'>
        ` +
          param.value +
          `
    </button>
...

    {
      ...
      cellRenderer(param) {
        return (
          `<button type='button' help='` +
          param.value +
          `' data-action-type='client' data-toggle="modal" class='btn  btn-sm'>
        ` +
          param.value +
          `
    </button>
    ...
    },
    {
      ...
      cellRenderer(param) {
        return (
          `<button type='button' help='` +
          param.value +
          `'  data-action-type='ClientProcess' data-toggle="modal" class='btn  btn-sm'>
        ` +
          param.value +
          `
    </button>
    ...
    },
    {
      ...
      cellRenderer(param) {
        return (
          `<button type='button' help='` +
          param.value +
          `'  data-action-type='Graduation' data-toggle="modal" class='btn  btn-sm'>
        ` +
          param.value +
          `
    </button>
  `
    ...
    },
    {
      ...
      cellRenderer(param) {
        return (
          `<button type='button'  data-action-type='Experience' data-toggle="modal" class='btn  btn-sm'>
        ` +
          param.value +
          `
    </button>
    ...
    },
    {
      ...
      cellRenderer(param) {
        ...
        return (
          `<button type='button' help='` +
          param.value +
          `'  data-action-type='Comment' data-toggle="modal" class='btn  btn-sm'>
        ` +
          param.value +
          `
    </button>
  `
    ...
    {
      headerName: 'Last Withdrawn Salary',
      width: 120,
      field: 'currentSalary',
      sortable: true,
      filter: true,
      cellRenderer(param) {
        return (
          `<button type='button' help='` +
          param.value +
          `'  data-action-type='changesalary' data-toggle="modal" class='btn  btn-sm'>
        ` +
          param.value +
          `
    </button>
  ...
  columnDefsmultiplecandidate = [
    {
      ...
      cellRenderer(param) {
        return (
          `<button type='button' data-action-type='multiplename' data-toggle="modal" class='btn  btn-sm'>
          ` +
          param.value +
          `
    </button>
    ...
    },
    {
      ...
      cellRenderer(param) {
        return (
          `<button type='button' help='` +
          param.value +
          `' data-action-type='multiplephone' data-toggle="modal" class='btn  btn-sm'>
      ` +
          param.value +
          `
  </span>
    ...
    },
    {
      ...
      cellRenderer(param) {
        return (
          `<button type='button' help='` +
          param.value +
          `' data-action-type='multipleemail' data-toggle="modal" class='btn  btn-sm'>
    ` +
          param.value +
          `
</span>
`
    ...
    },
    {
      ...
      cellRenderer(param) {
        return (
          `<button type='button' help='` +
          param.value +
          `'  data-action-type='multipleskill' data-toggle="modal" class='btn  btn-sm'>
        ` +
          param.value +
          `
    </button>
  `
...

    {
      ...
      sortable: true,
      filter: true,
      template: `<button type="button" data-action-type="Openresume" class="btn btn-success btn-sm">
       view Resume
     </button>`,
    },
  ];
  ...
  DownloadExcelFormate(id : any): void {
    this.db.downloadFile(
      'history/download-excel-format/',
      { trackerno: id },
      (response) => {
        response;
  ...
  private handleFileDownload(blob: Blob) {
    ...
    a.href = url;
    a.download = 'downloaded_file.xlsx'; // Adjust the filename as needed
    a.click();
  ...
  uploadresume(files: FileList) {
    ...
    this.db.storeupload(
      ...
      (response) => {
        this.viewresumee = response.Filename;
        this.db.showNotification('uploaded');
        $('#uploadresume').modal('show');
      },
      null,
      fileToUpload
  ...
  convertToExcel(jsonData:any, fileName:any) {
  ...
  uploadSelfexcel(files: FileList) {
    ...
    fileToUpload.push({ filekey: 'file', file: files.item(0) });
    this.db.storeupload(
      'candidate/Excel-candidate/',
      { file: files, trackerno: this.trackerid, source_id: '2' },
      (re) => {
        console.log("Re1 : " , re.failed_records);
        this.convertToExcel(re.failed_records, 'failed_records_output');

        this.LoadHistory();
        $('#uploadSelfexcel').modal('hide');
        $('#showcvs').modal('show');
        this.cvslists = re;
        this.db.showNotification(re.error);
      },
      (re) => {
        console.log("Re2 : " , re);
  ...
  filterhistory() {
    if (
      this.searchcandidatetext.length !== 0 ||
      this.searchcandidatetext.length !== undefined
    ) {
      const main = [];
      ...
      for (const i in this.candidatedetails) {
        for (const j in this.candidatedetails[i]) {
          if (
            this.candidatedetails[i][j] != null &&
            this.candidatedetails[i][j]
              .toString()
  ...
  CandidateHistoryStatus(): void {
    ...
    if (
      this.date != undefined &&
      this.date != null &&
      this.not_scheduled === 'Scheduled - Interview Aligned'
    ) {
      this.isinterview = 1;
      this.historystatus = {
        candidate_id: this.candidate_id,
      ...
      this.cvshareddate != undefined &&
      this.not_scheduled === 'CV Shared'
    ) {
  ...
  candidatesavemultiple(): void {
    ...
    this.db.list(
      'candidatesavemultiple/',
      {
        candidate_names,
        candidateemails,
        candidatephones,
        candidateskills,
        candidatemultipleids,
        candidateresumes,
      },
      (response): void => {
  ...
  OnRowclicked(evt:any) {
    switch (evt.headername) {
      case 'candidateProfile':
        const url = `/history/candidateProfile/${btoa(evt.fulldata.candidate_id)}`;
  ...
  openDialogWithTemplateRef(templateRef: TemplateRef<any>){
    ...
    dailogConfig.width = 'auto';
    this.dialog.open(templateRef,dailogConfig);
  }
