...
export class CampaignFlowComponent implements OnInit, AfterViewInit {
  ...
  newItem: CampaignFlowItem = {
    event_name: 'Start Campaign', event_order: 0, event_type: 'start', event: '', schedule_on: '', for: '', condition: '', name: '', color: '', trigger_interval_unit: '', trigger_interval: 0, jump_event: '0', trigger_date: '',
  };
  ...
  constructor(private route: ActivatedRoute, private repo: RepoService, public db: DbService, public router: Router, private fb: UntypedFormBuilder) { }
  setChartSize() {
    ...
    if (this.chart) {
      ...
      this.CampaignChartElementList = this.setCampaignChartElementList(this.list);
  ...
  isEnableDesableBot(opt: any, selected: any): boolean {
    return this.botcategories.value?.length >= selected && !this.botcategories.value?.find(el => el == opt);

  }
  isOptionDisabled(opt: any, selected: any, template): boolean {

    return this.categories.value?.length >= selected && !this.categories.value?.find(el => el == opt);

  }


  isOptionDisabledemailtemplate(opt: any, selected: any): boolean {
    return this.StoreSelectValue = this.categories1.value?.length >= selected && !this.categories1.value.find(el => el == opt);
  ...
  setCampaignChartElementList(list: CampaignFlowItem[]): CampaignChartElement[] {
  ...
  clickHandleNodeLayer(nodeLayer: CampaignChartElement) {
    ...
    localStorage.setItem('NodeInformation', JSON.stringify(this.selectedNodeLayers));
    const retrievedObject = localStorage.getItem('NodeInformation');
  }

  downHandleNodeLayer(pointerEvent: PointerEvent, nodeLayer: CampaignChartElement) {
    this.isDraggingGrid = false;
  ...
  setChildren(result: CampaignChartElement[], parent_id: CampaignChartElement, list: CampaignFlowItem[]) {
    const children = list.filter(item => item.parent_id === parent_id.item.id);
    this.selectedNodeLayers = [];
    parent_id.childrenCount = children.length;
    for (let i = 0; i < children.length; i++) {
      const child = children[i];
      const offset = parent_id.x - (children.length - 1) / 2 * (this.ELEMENT_WIDTH + this.PADDING * 2);
  ...
  shiftChartElements(list: CampaignChartElement[]): void {
    ...
    while (true) {
      let offset = 0;
      const elementsToShift = list.filter(e => e.item.event_order === eventOrder);
      ...
      for (let i = 0; i < elementsToShift.length - 1; i++) {
        ...
        const space = nextElement.x - this.ELEMENT_WIDTH - 5 * this.PADDING - currentElement.x;
        if (space < 0) {
          nextElement.x -= space;
          list.filter(e => e.parent_id === currentElement).forEach(child => child.x -= space);
          offset = Math.max(-space, offset);
        }
      }
      list.filter(e => e.item.event_order >= eventOrder).forEach(e => e.x -= offset / 2);
      eventOrder++;
    }
  }
    ...
    for (const element of list) {
      if (element.parent_id && element.item.event_order) {
        element.connection =
          ...
          `${element.parent_id.x + 11} ${element.parent_id.y + this.ELEMENT_HEIGHT}, ` +
          `${element.parent_id.x + 11} ${element.parent_id.y + this.ELEMENT_HEIGHT / 2}`;
      }
    }
  }

  compareElements = (a: CampaignChartElement, b: CampaignChartElement): number => {
  ...
  getCandidateStatus(): void {
    ...
    this.db.list('candidate-status/relations/' + 0, { allstatus: this.allstatusload }, (response): void => {
  ...
  getcampaignChannels(): void {
    ...
    this.allstatusload = 1;
    this.db.list('campaign/channel/', null, (response): void => {
  ...
  getChannelDetail(): void {
    ...
    const Foundchannel = this.campaignChannels.find(item => item.id === this.selected.channel_id);
  ...
  SaveConditionFlow(): void {
    ...
    this.db.store('campaign/flow/', this.selected, (response): void => {
  ...
  UpdateConditionFlow(): void {
    debugger;
    this.db.update('campaign/flow/', this.selected.id, this.selected, (response): void => {
      this.selected = response;
      if (this.selected.event_type == 'condition') {
        this.CloseModal('#condition-model');
      } else if (this.selected.event_type == 'action') {
  ...
  getChannelNameById(channelId: number): string {
    const selectedChannel = this.campaignChannels.find(channel => channel.id === channelId);
