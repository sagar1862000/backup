  ...
  async fetchChannelData() {
    ...
    try {
      const globalData1 = await this.httpClient
        .get<any>('http://192.168.4.115:8002/fetch_channels', { headers })
        .toPromise();
      const globalData2 = await this.httpClient
        .get<any>('http://192.168.4.115:8002/sendSMS', { headers })
        .toPromise();
      const globalData3 = await this.httpClient
        .get<any>('http://192.168.4.115:8002/sendBotName', { headers })
        .toPromise();
      const globalData4 = await this.httpClient
        .get<any>('http://192.168.4.115:8002/sendEmail', { headers })
        .toPromise();
      const globalData5 = await this.httpClient
        .get<any>('http://192.168.4.115:8002/candidate_statuses', { headers })
        .toPromise();
      debugger;
