...
export class JobCandidateComponent implements OnInit {
  ...
  DataDefinitions = [
    ...
    },
    {
      ...
      style: 'font-weight: bold; font-size:15px; border-radius:4px',
      print: 'bodyright',
    },
    {
      headerName: 'Activity',
      ...
      type:'button',
      style: 'font-weight: bold; font-size:15px; border-radius:4px',
      class: 'btn btn-sm btn-outline-primary col-md-12 bg-dark',
      tag: 'button',
      size: 'col-md-12',
      print: 'bodyright',
    },
  ...
  loadCandidate() {
    ...
    if (this.show_all_jobs === false) {
      ...
      this.db.globaljobid = parseFloat(SelectedJob);
      ...
      if (this.isinterview !== undefined) {
        Search.isinterview = this.isinterview.toString();;
      }
      ...
      this.db.list('jobs/candidates/', Search, (response): void => {
      ...
      this.rowData = [];
      // console.log(Search);
      this.db.list('jobs/candidates/', Search, (response:any): void => {
        this.candidatedetails = response;
        console.log('details : ' , response);
        this.candidateIds = response.map(item => item.id);
        console.log('Ids :' , this.candidateIds )
        this.rowData = response;
        this.gridOptionsloadcandidatesInPopUp.exporterAllDataFn = function () {
          return this.candidatedetails;
        };
...
;
  ...
  OnRowclicked(evt: any) {
    switch (evt.headername) {
      case 'candidateProfile':
        console.log('pass');
        const url = `myJob/candidateProfile/${btoa(evt.fulldata.id)}`;
        ...
        (r): void => {
          if (this.download) {
            if (r.No_Cv != undefined) {
              ...
              if (resumes.length > 0) {
                resumehtml =
                  '</br> <h3>Resume List</h3> <div class="list-group">';
                for (const k in resumes) {
                  if (resumes[k]) {
                    resumehtml +=
                      '<a target="_blank" class="list-group-item" href="https://api.passivereferral.com/resumes/' +
                      resumes[k].resume +
                      '">' +
                      resumes[k].name +
                      '</a>';
              ...
              this.db.showDialog(
                `Your excel is ready.<a href="https://api.passivereferral.com/trackers/` +
                  r.excel +
  ...
  Skipcandidate(download): void {
    ...
    const withresume = selectedcandidate.filter(function (obj) {
      return withoutresume.indexOf(obj) == -1;
    });
  ...
  uploadresume(files: FileList, updateid) {
    ...
    this.db.storeupload(
      'candidatedetail/update/' + updateid,
      null,
      (re) => {
    ...
    for (const key1 in evt.buttondetails1) {
      {
        switch (evt.buttondetails1[key1].key) {
          case 'more':
            ...
            this.coldef.push({
              data: evt.data,
              data_action_type: evt.buttondetails1[key1],
              data_root: evt.data.display_name,
            });
  ...
  public activity(data: any) {
    this.db.show('jobs/activity/', data.ajid, (response): void => {
