...
export class MyJobComponent implements OnInit {
  ...
  bindJob(): void {
    ...
    this.db.list('jobs/job-list/', { client_id: this.Client_ids, isclientdepartment: this.clientdepartment }, ((response): void => {
  ...
  assignjobClick(): void {
    ...
    if (selected === 1) {
      this.db.list('jobs/job-under-recruiters/', {
        job_id
      }, ((response): void => {
  ...
  tag(item): void {
    ...
    if (item.tagged == '10') {
      ...
      this.db.store('jobs/job-tag/', { job_id: item.id }, (response): void => {
        item.tagged = '11';
        this.db.showMessage('Tagged');
      });
    } else {

      this.db.destroy('jobs/job-tag/', item.id, (response): void => {
        item.tagged = '10';
  ...
  assignjob(): void {
    ...
    debugger;
    this.db.store('jobs/assign-job/', assignjob, ((response): void => {
      // console.log(response);
      this.db.addmessageandremove('job successfully asssign');
    }));
  }
  ...
  unassignjob(): void {
    ...
    this.db.store('jobs/unassign-job/', unassignjob, ((response): void => {
      // console.log(response);
