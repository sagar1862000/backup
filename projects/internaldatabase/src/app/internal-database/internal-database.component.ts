...
export class InternalDatabaseComponent implements OnInit {
  ...
  async onInputChangeAny(event: Event) {
    ...
    try {
      const globalData1 = await this.httpClient
        .get<any>('http://192.168.4.100:8080//query_suggestions', {
          headers,
          params,
        })
        .toPromise();
  ...
  loadlocation(): void {
    ...
    this.location = [];
    this.internalDb.list('search/location_dropdown', null, (response): void => {
  ...
  onLocationSelectionChange(event: any) {
    // Clear the array before pushing the selected values if you don't want duplicates
    this.selectedLocations = [];
    // Push selected values into the array
  ...
  public loadGridBoolean(): void {
    ...
    const urladvance = this.router.createUrlTree([
      'internaldatabaseadvance/booleansearch',
      {
        SearchType: 'boolean',
